...
- `src/extract_dvcs_cff/cli/user.py`: configurable workspace/database roots,
  dedicated corpus/export roots, project-only workspace listing, recorded
  scheduler environment overrides, public `./dvcs` command hints, and
...
- `src/extract_dvcs_cff/corpus.py`: serialize shared realization publication
  and reuse a fully matching corpus/selection/configuration/bridge identity
  across multi-GPU worker processes;
...
- `tools/sync_upstream.py`: support recording a reviewed three-way packaging
  merge while preserving distinct upstream and distribution hashes
  (packaging-owned);
